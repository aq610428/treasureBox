...
public class VehicleActivity extends BaseActivity implements NetWorkListener {
    ...
    private void query() {
        ...
        String vinno = text_frame.getText().toString().trim();//车架号码
        String engineno = text_enginee.getText().toString().trim();//发动机号
        ...
        if (!Utility.isEmpty(yearmodelid)) {
            sign = sign + "&yearmodelid=" +yearmodelid + Constants.SECREKEY;
        } else {
